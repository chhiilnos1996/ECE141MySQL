  ...
  public:
    ...
    static const char* keywordToString(Keywords aType) {
      switch(aType) {
        ...
        case Keywords::where_kw:      return "where";
    ...
    static bool isNumericKeyword(Keywords aKeyword) {
      ...
      static StatusResult stringValueToDataType(DataTypes aType, const std::string& aStringValue, Value& aValue) {
           static std::map<DataTypes, std::function<Value(std::string aValue)>> theTypeConversions = {
             {DataTypes::bool_type, [&](std::string aValue) {
               return (aValue == "true");
             }},
             {DataTypes::datetime_type, [&](std::string aValue) {
               // TODO: validate datetime
               return aValue;
             }},
             {DataTypes::float_type, [&](std::string aValue) {
               return std::stod(aValue);
             }},
             {DataTypes::int_type, [&](std::string aValue) {
               return std::stoi(aValue);
             }},
             {DataTypes::varchar_type, [&](std::string aValue) {
               return aValue;
             }}
           };

           if (theTypeConversions.count(aType)) aValue = theTypeConversions[aType](aStringValue);
         ...
         static std::string valueToString(const Value& aValue) {
           return std::visit(ValueConversion{}, aValue);
         }
         
         static std::string QueryOk(size_t aCount, size_t anElapsed) {
           return "Query Ok, " + std::to_string(aCount) + " rows affected (" + std::to_string(anElapsed) + " secs)\n";
         }
         
         static std::string QueryOk(size_t aCount, double anElapsed) {
           std::stringstream theStream;
           theStream << "Query Ok, " << std::to_string(aCount) << " rows affected (" << std::fixed << std::setprecision(2) << anElapsed << " secs)\n";
